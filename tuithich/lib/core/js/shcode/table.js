function addTableBuilder(){
	...
	if(validated){
		...
		output += '<div><input type="button" onclick="addTableCode('+rows+','+cols+','+'\''+jQuery('#table_title').val()+'\''+')" id="insert_tabs_btn" value="'+__('Add Table to the post')+'" class="button-primary"></div>';
...
function addTableCode(rows,cols,title){
	...
	if(jQuery('#table_title').val() != ''){
		table += '<thead><tr><th colspan='+cols+'>'+jQuery('#table_title').val()+'</th></tr></thead>';	
	}
    table += '<tbody>';
	for(var i = 1; i<=rows; i++){
